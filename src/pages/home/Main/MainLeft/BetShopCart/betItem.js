...
@connect(({ matchDB, oddsDB, chsDB, betShopCart, changeBetSectionStatus }) => ({
  matchDB,
  oddsDB,
  chsDB,
      ...
      return (
        <Fragment>
          <div className={styles['bet-item-header']}>
            <Icon className={styles.del} onClick={() => this.delShopCartItem(data.choiceId)}
                  type="close-circle"/>
            ...
            <div className={styles.winInfo}>
              <div className={styles['bet-select']} ref={this.selectRef}>
                ...
                <div className={styles['bet-select-arrow']} onClick={this.selectAmount}>
                  <Icon className={showSelectOption ? `${styles.icon} ${styles.active}` : styles.icon}
                        type="down"/>
                </div>
                {
                  showSelectOption ? (
        ...
        <Fragment>
          <div className={styles['no-bet']}>
            <div className={styles.left}>
              <span className={styles.text}>注意：</span><span className={styles.result}>{data.message}</span>
            </div>
    ...
    if (data.code === '3001' || data.code === '2111' ) {
      return (
        <Fragment>
          <div className={styles['no-bet']}>
            <div className={styles.left}>
              <span className={styles.text}>该投注项当前不可投注：</span><span className={styles.result}>{data.message}</span>
            </div>
            <div className={styles.right}>
              <div className={styles.del} onClick={() => this.delShopCartItem(data.choiceId)}>X</div>
            </div>
